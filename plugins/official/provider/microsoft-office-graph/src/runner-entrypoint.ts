...
startStdioJsonRuntime({
  pluginName: 'microsoft-office-graph',
  version: '0.1.0',
  capabilities: ['microsoft.office.graph.read', 'microsoft.office.graph.write', 'microsoft.office.graph.delete'],
  async handleInvoke({ capability, input }) {
    ensureToken();

    if (capability === 'microsoft.office.graph.read') {
    ...
    if (capability === 'microsoft.office.graph.write') {
    ...
    if (capability === 'microsoft.office.graph.delete') {
      const path = resolveDriveItemPath(input, true);
