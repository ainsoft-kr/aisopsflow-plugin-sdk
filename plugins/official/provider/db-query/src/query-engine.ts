...
export async function handleDbInvoke(capability: string, input: any) {
  const req = capability === 'db.query' ? validateLegacyQueryRequest(input || {}) : validateReadRequest(input || {});
  const ds = resolveDatasource(req.datasource, req.driver);
  if (capability === 'db.query') return runRead(ds, req);
...
export function validateLegacyQueryRequest(body: any) {
  if (!body || typeof body !== 'object') fail('request body must be an object');
  if (typeof body.dsn_ref !== 'string' || !body.dsn_ref.trim()) fail('dsn_ref is required');
  if (!['postgres', 'mysql'].includes(body.driver)) fail('driver must be postgres or mysql');
  if (typeof body.query !== 'string' || !body.query.trim()) fail('query is required');
  if (body.read_only === false) fail('db.query only supports read_only=true');
  if (body.params !== undefined && typeof body.params !== 'object') fail('params must be an object when provided');
  if (/[;]/.test(body.query)) fail('multi-statement execution is not allowed');
  if (!isReadOnlyStatement(body.query)) fail('only read-only statements are allowed');

  return {
    datasource: body.dsn_ref.trim(),
    driver: body.driver,
    statement: body.query.trim(),
    params: [],
