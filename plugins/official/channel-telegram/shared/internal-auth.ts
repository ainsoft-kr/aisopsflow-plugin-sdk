...
export function requireInternalAuthJson(req, res, next, internalAuthToken) {
  ...
  const expected = renderInternalSignature(internalAuthToken, timestamp, req.method, req.originalUrl || req.url || '/', req.rawBody || '');
...
export function signedInternalHeaders(token, method, originalUrl, bodyText, contentType = 'application/json') {
  const headers = {};
  if (contentType) {
  ...
  const signature = renderInternalSignature(token, timestamp, method, originalUrl, bodyText);
...
export function renderInternalSignature(token, timestamp, method, originalUrl, bodyText) {
